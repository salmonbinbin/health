...
export const authApi = {
  ...
  setAuthInfo(authData) {
    ...
    if (authData.user) {
      // 检查用户是否被禁用
      if (authData.user.isDisabled === true) {
        console.log('检测到禁用用户，设置禁用标记');
...
api.interceptors.request.use(
  config => {
    ...
    // 检查用户是否被禁用
    const isDisabled = localStorage.getItem('userDisabled') === 'true' || 
...
api.interceptors.response.use(
  response => {
    ...
    if (response.data && 
      ...
      console.warn('检测到禁用用户登录成功响应，设置禁用标记');
      localStorage.setItem('userDisabled', 'true');
